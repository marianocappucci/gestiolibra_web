"""
Backend de acceso a /docs/ para la landing de Gestiolibra.

No guarda usuarios propios: cada login se valida en tiempo real contra la
instancia real del cliente (POST https://{subdominio}.gestiolibra.com.ar/auth/verify),
reutilizando el auth que ya existe en cada contenedor (libracore.auth.SessionAuth +
tabla users propia). A diferencia de Contalibra/Restolibra, Gestiolibra no tiene un
...
patron que ya usan contalibra_web/restolibra_web contra sus respectivas apps.
"""
import os
import re
...
SECRET_KEY       = os.environ.get("DOCS_SESSION_SECRET", "gestiolibra-docs-secret-change-me")
APEX_DOMAIN      = os.environ.get("APEX_DOMAIN", "gestiolibra.com.ar")
...
app = FastAPI(title="Gestiolibra Docs Auth", docs_url=None, redoc_url=None)
...
<head>
<meta charset="UTF-8">
<title>Acceso a Documentación · Gestiolibra</title>
...
<style>
  ...
  button {{ width:100%; margin-top:1.25rem; padding:.7rem; background:#7c3aed; color:#fff;
    ...
    <form method="post" action="/login-docs">
      ...
      <div class="domain-row">
        <input type="text" name="slug" id="slug" required autocomplete="off"
               placeholder="tu-negocio" value="{slug}">
