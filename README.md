# Gestiolibra — Sitio Web / Landing

Landing page de marketing para [Gestiolibra](https://gestiolibra.com.ar), turnos y
agenda para negocios de servicios (barberías, peluquerías, estética, lavaderos,
talleres). Mismo patrón que `contalibra_web`/`restolibra_web`: HTML estático
...
- **Documentación técnica en `/docs/`** (12 páginas), gateada por login real
  ...
  diferencia: como Gestiolibra no tiene un backoffice con `/api/clientes-publicos`,
...
- **CI/CD** (`.github/workflows/deploy.yml`): push a `main` dispara rsync al
  ...
  repo (no incluido — mismo criterio que el resto de la familia, el asistente
  no gestiona claves privadas).
- **Todavía sin**: primer deploy real al VPS, ni proxy NPM/SSL para el apex
  `gestiolibra.com.ar` (hoy solo existe para `dev.gestiolibra.com.ar`, la app).
...
Sirve en `http://localhost:8083` (requiere la red externa `stack_stack-net` —
...
El backend `auth/` valida en tiempo real contra `https://{subdominio}.gestiolibra.com.ar/auth/verify`
(nuevo endpoint necesario del lado de la app, ver `gestiolibra` repo) usando el
...
docker-compose.yml      — servicios web (8083:80) + auth (interno), red stack_stack-net
.github/workflows/
...
- Producto documentado: [Gestiolibra](https://github.com/marianocappucci/gestiolibra)
- Mismo patrón: `contalibra_web`, `restolibra_web`
